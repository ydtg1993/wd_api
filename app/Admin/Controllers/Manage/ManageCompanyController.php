...
class ManageCompanyController extends AdminController
{
    /**
    ...
    {
        $grid = new Grid(new MovieFilmCompanies);
        ...
        $grid->column('name', '片商名称')->display(function ($v) use ($text_style) {
        ...
        $url = config('app.url') . '/inner/manage_company';
    ...
    public function update($id)
    {
        ...
        try {
            ...
            $selected = MovieFilmCompaniesAss::where('film_companies_id', $id)->pluck('mid')->all();
            list($insert, $delete) = CommonController::dotCalculate($selected, $data['numbers']);
            if (!empty($insert)) {
                foreach ($insert as &$v) {
                    $mid = $v;
                    $v = ['mid' => $mid, 'film_companies_id' => $id];
                }
                MovieFilmCompaniesAss::insert($insert);
            }
            if (!empty($delete)) {
                MovieFilmCompaniesAss::where('film_companies_id', $id)->whereIn('mid', $delete)->delete();
            }
            MovieFilmCompanies::where('id', $id)->update([
                'name' => $data['name'],
                'status' => $data['status'],
                'cid' => $data['cid'],
                'movie_sum' => MovieFilmCompanies::where('series_id', $id)->count()
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            return DLPViewer::result(false, $e->getMessage());
    ...
    {
        $form = new Form(new MovieFilmCompanies);
