...
class ManageNumbersController extends AdminController
{
    /**
    ...
    protected function grid()
    {
        ...
        $url = config('app.url') . '/inner/manage_numbers';
    ...
    public function update($id)
    {
        ...
        try {
            ...
            $selected = MovieNumberAss::where('nid', $id)->pluck('mid')->all();
            ...
            if (!empty($insert)) {
                ...
                MovieNumberAss::insert($insert);
            }
            if (!empty($delete)) {
                MovieNumberAss::where('series_id', $id)->whereIn('mid', $delete)->delete();
            }
            MovieNumber::where('id', $id)->update([
                'name' => $data['name'],
                'status' => $data['status'],
                'movie_sum' => MovieNumberAss::where('nid', $id)->count()
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            return DLPViewer::result(false, $e->getMessage());
